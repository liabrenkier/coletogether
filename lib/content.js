...
const groqQuery = `
*[_type in ["salida", "viaje"] && defined(fechaInicio) && coalesce(oculta, false) == false] | order(fechaInicio asc) {
  _id,
  _type,
  titulo,
  "slug": coalesce(slug.current, _id),
  "destacadaInicio": coalesce(destacadaInicio, false),
  "oculta": coalesce(oculta, false),
  fechaInicio,
...
function normalizeEvent(item) {
  ...
  return {
    ...
    destacadaInicio: Boolean(item.destacadaInicio),
...
async function fetchFromSanity() {
  ...
  const response = await fetch(endpoint, {
    ...
    next: { revalidate: 300, tags: ["cole-content"] }
  });
